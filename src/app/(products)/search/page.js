...
const getData = async (limit, offset, order, hotsale, inStock) => {
  const data = await Ask(
    `${BaseConfig.apiUrl}/products/list/${limit}/${offset}/${order}/${hotsale}/${inStock}`,
    "get",
    "no-store"
  );
  return data;
...
const Search = async ({ searchParams }) => {
  ...
  const data = await getData(limit, offset, order, hotsale, inStock);
